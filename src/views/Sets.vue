<template>
  <div class="home">
    ...
    <div class="sets-list container" v-if="active === 'list'">
      <div v-for="(set, key) of sets" v-bind:key="key">
        <div @click="redirect('/set/' + key)">
          <div class="card is-one-third">
            <div class="card-content">
              <div class="media">
                <div class="media-content">
                  <p class="title is-4">{{ getSetTitle(set) }}</p>
              ...
              <div class="content">
                <i>{{ getSetDescription(set).substring(0, 114) }}...</i>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getSetTitle(set) {
      console.log("set", set);
      if (set._metadata && set._metadata.name) {
        return set._metadata.name;
      } else {
        return "Collection...";
      }
    },
    getSetSubtitle(set) {
      if (set._metadata && set._metadata.Subtitle) {
        return set._metadata.Subtitle;
      } else {
        return "";
      }
    },
    getSetDescription(set) {
      if (set._metadata && set._metadata.Description) {
        return set._metadata.Description;
      } else {
        return "";
      }
    }
