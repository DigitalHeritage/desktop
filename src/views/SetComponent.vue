<template>
  <div>
    ...
    <div class="cards-container" v-if="active === 'browse'">
      <div
        class="card"
        v-for="(set, index) in filteredList"
        :key="`${index}`"
          ...
          <div class="media">
            ...
            <div class="media-content">
              <div class="media-content-inner">
                ...
                >
                  <div v-if="set._metadata">
                    <p class="subtitle is-7">
                      {{ set._metadata.Subtitle }}
                    </p>
                    <p class="title is-6">{{ set._metadata.Title }}</p>
                  </div>
                </router-link>
              </div>
            </div>
    ...
    >
      ...
      <article style="margin-top:30px;" class="message is-gray">
        ...
        <div class="message-body">
          <div class="content">
            <p>https://api.digitalheritage.fr/sets-guardian.php</p>
          </div>
        </div>
      </article>
    </div>
...
export default {
  ...
  computed: {
    filteredList() {
      let filteredArray = [];
      console.log(typeof this.sets[0]);
      for (let index = 0; index < this.sets.length; index++) {
        if (this.sets[index]._metadata != undefined) {
          if (this.sets[index]._metadata.Title != undefined) {
            if (
              this.sets[index]._metadata.Title.toLowerCase().includes(
                this.search.toLowerCase()
              )
            ) {
              //Save the oldIndex in metadata to prevent from messing with URLs
              let elem = this.sets[index];
          ...
          console.log(this.sets[index]);
          this.createMetadata(this.sets[index]);
          if (
            this.sets[index]._metadata.Title.toLowerCase().includes(
              this.search.toLowerCase()
            )
  ...
  methods: {
    loadFromDB() {
      ...
      Vue.prototype.$Sets[this.setId].data = ans;
      Vue.prototype.$Sets[this.setId]._metadata = ansMetadata;
      this.sets = ans;
    ...
    saveInDB() {
      ...
      console.log(JSON.stringify(this.$Sets[this.setId]));
      xmlHttp.send(JSON.stringify(this.$Sets[this.setId]));
    },
    ...
    downloadJson() {
      let filename = this.metadata.filename
        ...
        : "set.json";
      downloadTextFile(
        JSON.stringify({ _metadata: this.metadata, data: this.sets }),
        filename
      );
    },
    loadURL() {
      // To test, use : https://api.digitalheritage.fr/sets-guardian.php'
    ...
    deleteCollection() {
      this.$Sets.splice(this.setId, 1);
      this.$router.push("/");
    },
    addAnItem() {
      let lastnum = this.sets.length - 1;
      // Cloning the last item into an object
      let item = Object.assign({}, this.sets[lastnum]);
      console.log(item);
  ...
  created() {
    this.setId = this.$route.params.id ? this.$route.params.id : 0;
    this.sets = this.$Sets[this.setId].data;
    if (this.sets === undefined) {
      this.sets = [];
    }
    //console.log(this.sets);
    if (this.$Sets[this.setId]._metadata) {
      this.metadata = this.$Sets[this.setId]._metadata;
    } else {
      this.metadata = {
        Title: "Collection...",
        Subtitle: "",
    ...
    for (let i = 0; i < this.sets.length; i++) {
      ...
      console.log("_metadata", this.sets[i]._metadata);
      // Geo is inside _metada.Geo
      // TODO : group by same Geo, for Paris Louvre
      if (
        this.sets[i]._metadata &&
        this.sets[i]._metadata.Geo &&
        this.sets[i]._metadata.Geo[0]
      ) {
        console.log("geo0", this.sets[i]._metadata.Geo[0]);
        this.markers.push({
          geo: L.latLng(
            this.sets[i]._metadata.Geo[0],
            this.sets[i]._metadata.Geo[1]
          ),
          name: this.sets[i]._metadata.Title,
          id: i,
          image: this.sets[i]._metadata.Image
        });
      }
    }
  }
